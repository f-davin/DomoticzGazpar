...
class BasePlugin:
    ...
    def createDevice ( self ):
        ...
        if not self.iIndexUnit in Devices:
            Domoticz.Device ( Name = self.sDeviceName, Unit = self.iIndexUnit, Type = self.iType,
                              Subtype = self.iSubType,
                              Switchtype = self.iSwitchType, Description = self.sDescription, Used = 1
                              ).Create ( )
            if not (self.iIndexUnit in Devices):
                Domoticz.Error (
    ...
    def login ( self ):
        ...
        if json_content [ 'state' ] != "SUCCESS":
            ...
            Domoticz.Error ( 'An error occurs during login: \n\t' + error + '\n' )
